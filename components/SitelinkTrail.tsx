...
export function SitelinkTrail({ path }: { path: string }) {
  const route = routes.find((candidate) => candidate.path === path);
  const sitelinkName =
    route && "sitelinkName" in route ? route.sitelinkName : undefined;
  ...
  return (
    <>
      <nav
        ...
        className="container-wide pt-6 text-sm text-[#6e6e73]"
      >
